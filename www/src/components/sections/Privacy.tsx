...
const STAYS = [
  { icon: D.doc, name: "Documents", sub: "Read-only" },
...
const LANES = [
  ...
  },
  {
    key: "excerpts",
    icon: D.spark,
    title: "Excerpts + brief",
    dest: "Your model provider",
    note: "Only when it answers",
    line: "pv-line-dashed",
    dot: "bg-muted",
    burst: true,
    card: "border-border",
      ...
      <div className="page-column grid items-center gap-12 lg:grid-cols-2">
        <FadeInUp>
          ...
          <p className="mt-6 max-w-md text-sm leading-relaxed text-muted">
            Your documents, indexes and brief never leave this Mac. Two things do: meeting audio,
            streamed to the transcription provider you configure, and — only when Savvy answers —
            excerpts, the brief and recent turns, sent to the model your CLI is signed in to.
          </p>
          <StatList
            items={[
              { icon: FolderIcon, label: "Documents read-only, never copied" },
              { icon: ClockIcon, label: "Audio and transcripts deleted after 30 days" },
              { icon: WaveIcon, label: "Audio streams to your transcription provider" },
            ]}
          />
        </FadeInUp>
