...
Wrangler serves `out/` as Worker static assets. `worker.js` redirects
`savvycopilot.com` to `www.savvycopilot.com` with a 308 and serves everything else
from the assets.
The CLI version is pinned in the scripts and downloaded by pnpm when needed.

```sh
pnpm dlx --allow-build=esbuild --allow-build=workerd wrangler@4.127.1 login
pnpm dlx --allow-build=esbuild --allow-build=workerd wrangler@4.127.1 whoami
...
Never commit credentials. Custom-domain routes cover `www.savvycopilot.com` and
...
robots.txt, the sitemap and llms.txt. It is set to `https://www.savvycopilot.com`.
